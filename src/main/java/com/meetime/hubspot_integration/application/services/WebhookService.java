...
public class WebhookService implements WebhookUseCase {
    ...
    public void processWebHookEvents(String payload) throws Exception {
        List<WebhookEventDTO> contactEventPayloads = WebhookEventMapper.webhookEventConverter(payload);

        for (WebhookEventDTO contactEventPayload : contactEventPayloads) {
            Webhook webhookEvent = new Webhook();
